...
#define ENEMY_MOVE_SPEED 2		//敵の動く速さ
...
int g_EnemyMove = SCREEN_WIDTH;	// 敵の動き用
int g_MoveReverse = -1;			// 端や壁に当たった際の反転
...
{
	// 移動反転処理
	if(g_EnemyMove < 0) g_MoveReverse = ENEMY_MOVE_SPEED;
	if(g_EnemyMove > SCREEN_WIDTH-BLOCKSIZE) g_MoveReverse = -ENEMY_MOVE_SPEED;

	g_EnemyMove += g_MoveReverse;

	DrawGraph(g_EnemyMove,300,Enemy_Image[0],TRUE);
...
int LoadImages(void)
{
	...
	//ナンバー画像の読み込み（分割）
		if(LoadDivGraph("Image/mob.png",20,10,2,32,48,Enemy_Image)==-1)return -1;//分割、横、縦、幅、高さ
	return 0;
}
